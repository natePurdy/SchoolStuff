...
CLASSES: diffferent types on NN's, most have similar needs in terms of training loop, except spikedCNN, which needs steps

OUTPUT: training history csv, and plots of confusion matrix and training, in a folder with the model name in it,
where ever this script is ran from...
"""


# some important high level parameters...
numEpochs = 10
learningRate = 0.001 # lower learning rate if using spiked NN
batchSize = 128
...
# for running on the grid and stuff
# for running on the grid and stuff
def parseArgs():
    parser = argparse.ArgumentParser(description="Train CIFAR NN on HPC")
    
    # Data paths (allow overrides)
    parser.add_argument('--dataFolder', type=str, default="/home/u25/nathanielpurdy/ece523/trainingData/cifar-10-batches-py/")
    # Training params
    parser.add_argument('--numEpochs', type=int, default=1)
    parser.add_argument('--batchSize', type=int, default=128) # maybe make this even larger once it works correctly on HPC
    parser.add_argument('--learningRate', type=float, default=1e-3, help="Learning rate")
    parser.add_argument('--augmentTrainingData', action='store_true', default=True, help="Enable data augmentation")
    parser.add_argument('--seed', type=int, default=12345, help="Random seed")
    parser.add_argument('--outputFolder', type=str, default="/home/u25/nathanielpurdy/ece523/outputData/", help="Output dir; auto-generated if None")
    parser.add_argument('--imageRows', type=int, default=32)
    parser.add_argument('--imageCols', type=int, default=32)


    
    # Model/optim tweaks (optional)
...
class DeeperViT(nn.Module):
    def __init__(self, input_shape=(3, 32, 32), num_classes=10,
        ...
        self.num_patches = (height // patch_size) ** 2   # e.g. 64 for 3232 & patch=4
...
#         # Simplest practical way to define custom block (recommended)
...
class SpikingResidualBlock(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size=3, padding=1, beta=0.9, spike_grad=None, threshold=0.4):
        super().__init__()
        self.conv1 = nn.Conv2d(in_channels, out_channels, kernel_size, padding=padding)
        self.bn1   = nn.BatchNorm2d(out_channels)
        self.lif1  = snn.Leaky(beta=beta, threshold=threshold, learn_threshold=True, spike_grad=spike_grad)
        
        self.conv2 = nn.Conv2d(out_channels, out_channels, kernel_size, padding=padding)
        self.bn2   = nn.BatchNorm2d(out_channels)
        self.lif2  = snn.Leaky(beta=beta, threshold=threshold, learn_threshold=True, spike_grad=spike_grad)

        # 1x1 conv for skip connection if channel mismatch
        self.shortcut = None
    ...
    def forward(self, spk_in, mem_in):
        # First conv + LIF
        cur = self.conv1(spk_in)
        cur = self.bn1(cur)
        spk_mid, mem_mid = self.lif1(cur, mem_in)
        
        # Second conv + LIF
        cur = self.conv2(spk_mid)
        cur = self.bn2(cur)
        spk_out, mem_out = self.lif2(cur, mem_mid)
        
        # Residual addition on spikes (most common in SNNs)
        identity = spk_in
        if self.shortcut is not None:
            identity = self.shortcut(identity)
        
        spk_out = spk_out + identity
        
        return spk_out, mem_out  # return both for next block

class SpikingDeeperCNN(nn.Module):
    def __init__(self, input_shape=(3,32,32), num_classes=10, num_steps=10, beta=0.85):
        super().__init__()
        self.num_steps = num_steps
        self.beta = beta
        slope = 3  # Lower for wider surrogate (helps gradients flow)
...
class SpikingDeeperCNNResnet(nn.Module):
    def __init__(self, input_shape=(3,32,32), num_classes=10, num_steps=5, beta=0.9):
        super().__init__()
        self.num_steps = num_steps
        self.beta = beta
        slope = 2
        self.spike_grad = surrogate.fast_sigmoid(slope=slope)
        self.spikeThreshold = 0.7
        
        # Stem (initial conv to get to 32 channels)
        self.stem = nn.Sequential(
            nn.Conv2d(3, 32, 3, padding=1),
            nn.BatchNorm2d(32)
        )
        
        # Now use residual blocks
        self.block1 = SpikingResidualBlock(32, 32, beta=beta, spike_grad=self.spike_grad, threshold=self.spikeThreshold)
        self.block2 = SpikingResidualBlock(32, 32, beta=beta, spike_grad=self.spike_grad, threshold=self.spikeThreshold)
        
        self.pool1 = nn.MaxPool2d(2)
        
        self.block3 = SpikingResidualBlock(32, 64, beta=beta, spike_grad=self.spike_grad, threshold=self.spikeThreshold)
        self.block4 = SpikingResidualBlock(64, 64, beta=beta, spike_grad=self.spike_grad, threshold=self.spikeThreshold)
        
        self.pool2 = nn.MaxPool2d(2)
        
        self.block5 = SpikingResidualBlock(64, 128, beta=beta, spike_grad=self.spike_grad, threshold=self.spikeThreshold)
        self.block6 = SpikingResidualBlock(128, 128, beta=beta, spike_grad=self.spike_grad, threshold=self.spikeThreshold)
        
        self.pool3 = nn.MaxPool2d(2)

        flattened_size = 32 * 2**2 * 2**2 * 2**2   # dependds on input size and number of pools 
        ...
        self.fc3 = nn.Linear(128, num_classes)
        self.lif_fc3 = snn.Leaky(beta=beta, threshold=self.spikeThreshold, learn_threshold=True, spike_grad=self.spike_grad)

    def forward(self, x):
        mem1_1 = self.lif_fc1.init_leaky()  # reuse for convenience or init per block
        mem1_2 = self.lif_fc1.init_leaky()
        mem2_1 = self.lif_fc1.init_leaky()
        mem2_2 = self.lif_fc1.init_leaky()
        mem3_1 = self.lif_fc1.init_leaky()
        mem3_2 = self.lif_fc1.init_leaky()
        mem_fc1 = self.lif_fc1.init_leaky()
        mem_fc2 = self.lif_fc1.init_leaky()
        mem_fc3 = self.lif_fc1.init_leaky()
        
        spk_out_rec = []
        mem_out_rec = []
        
        for step in range(self.num_steps):
            cur = torch.poisson(x * 100)
            
            # Stem
            cur = self.stem(cur)
            
            # Residual blocks to pass spikes and membrane
            spk, mem1_1 = self.block1(cur, mem1_1)   # note: we pass spikes as input
            spk, mem1_2 = self.block2(spk, mem1_2)
            cur = self.pool1(spk)
            
            spk, mem2_1 = self.block3(cur, mem2_1)
            spk, mem2_2 = self.block4(spk, mem2_2)
            cur = self.pool2(spk)
            
            spk, mem3_1 = self.block5(cur, mem3_1)
            spk, mem3_2 = self.block6(spk, mem3_2)
            cur = self.pool3(spk)
...
def trainTheArtificialNN(model, train_loader, val_loader, test_loader, optimizer):
    ...
    for epoch in range(numEpochs):
        ...
        # Inside epoch loop, after validation, save model checkpoint
        checkpoint = {
            'epoch': epoch,
        ...
        torch.save(checkpoint, os.path.join(outputFolder, f"checkpoint_epoch_{epoch+1}.pth"))
...
def trainTheNaturalNN(model, train_loader, val_loader, test_loader, optimizer):
    ...
    for epoch in range(numEpochs):
        ...
        for images, labels in train_bar:
            images, labels = images.to(device), labels.to(device)
            optimizer.zero_grad()
            spk_rec, mem_rec = model(images)  # can either use spikes or membranes
            ...
            # Use membranes for predictions (membrane decoding)
            mem_sum = mem_rec.sum(dim=0)
        ...
        with torch.no_grad(): # no backpropping here...
            for images, labels in val_loader:
                images, labels = images.to(device), labels.to(device)
                ...
                mem_sum = mem_rec.sum(dim=0)
                preds = mem_sum.argmax(dim=1) # use the membrane not spikes for predicitions
                val_correct += (preds == labels).sum().item()
                val_total += labels.size(0)
    ...
    with torch.no_grad():
        for x, y in test_loader:
            ...
            mem_sum = mem_rec.sum(dim=0) # also using membrane potentials for val
            preds_all.extend(mem_sum.argmax(dim=1).cpu().numpy())
            labels_all.extend(y.numpy())
...
if __name__=="__main__":
    ...
    # make it work on the HPC...
    args = parse_args()
    # Override script vars with args
    dataFolder = args.dataFolder
    numEpochs = args.numEpochs
    batch_size = args.batchSize
    set_global_seed(args.seed)  # Use your seed function
    numImageRows = args.imageRows
    numImageCols = args.imageCols
    outputFolder = ars.outputFolder
    image_size = (numImageCols, numImageRows)   # 32x32 colour image
    ...
    model = SpikingDeeperCNNResnet().to(device)                                                 # <------------------- set model here
    ...
    testName = f"model{model.__class__.__name__}_"
    # Auto-generate output folder with timestamp 
    from datetime import datetime
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    outputFolder = f"testResults_{testName}_{timestamp}"
    ...
    ########### DO TRAINING HERE ##############
    if model.__class__.__name__ == "SpikingDeeperCNN" or model.__class__.__name__ == "SpikingDeeperCNNResnet":
        # slightly different training paradigm than regular CNN or transformer training
        optimizer = optim.AdamW(model.parameters(), lr=learningRate, weight_decay=0.05)  # optional small weight decay
